...
async def terminal_ws(websocket: WebSocket, vm_id: int):
    ...
    try:
        ...
        tasks = [asyncio.create_task(read_child()), asyncio.create_task(read_ws())]
...
@router.websocket("/console/{vm_id}")
async def console_ws(websocket: WebSocket, vm_id: int):
    payload = _verify_ws_token(websocket.query_params.get("token"))
    if not payload:
        await websocket.close(code=4001, reason="Token inválido")
        return
    ...
    async with async_session() as session:
        ...
        if not vm_name:
            await websocket.close(code=4004, reason="VM sin nombre")
            return
    ...
    child = None
    try:
        cmd = "virsh console --force {}".format(vm_name)
        child = pexpect.spawn(cmd, timeout=None, encoding="utf-8", codec_errors="replace")
        child.delaybeforesend = 0.01
